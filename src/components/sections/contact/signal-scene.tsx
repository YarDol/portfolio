...
function buildRings(group: THREE.Group): { rings: THREE.Line[]; ringMats: THREE.LineBasicMaterial[] } {
  const ringMats = Array.from({ length: RING_COUNT }, () => new THREE.LineBasicMaterial({ transparent: true }));
  ...
  return { rings, ringMats };
}

function buildNodes(
  group: THREE.Group,
  nodeMat: THREE.MeshBasicMaterial,
): { nodes: THREE.Mesh[]; nodePositions: THREE.Vector3[]; connLines: { line: THREE.LineSegments; posArr: Float32Array; geo: THREE.BufferGeometry }[] } {
  const nodePositions = Array.from({ length: NODE_COUNT }, (_, i) => {
    const angle = (i / NODE_COUNT) * Math.PI * 2;
    const r = 2.5 + Math.sin(i * 1.3) * 0.8;
    return new THREE.Vector3(Math.cos(angle) * r, Math.sin(angle) * r * 0.6, Math.sin(i * 0.9) * 0.5);
  });
  ...
  return { nodes, nodePositions, connLines };
}
...
export function SignalScene() {
  ...
  useEffect(() => {
    ...
    const glowMat = new THREE.MeshBasicMaterial({ transparent: true, opacity: 0.08, side: THREE.BackSide });
    const particleMat = new THREE.PointsMaterial({ size: 0.04, transparent: true, opacity: 0.35, sizeAttenuation: true });
    const nodeMat = new THREE.MeshBasicMaterial({ transparent: true, opacity: 0.6 });

    const nucleus = new THREE.Mesh(new THREE.SphereGeometry(0.42, 48, 48), nucleusMat);
    group.add(nucleus);
    group.add(new THREE.Mesh(new THREE.SphereGeometry(0.65, 32, 32), glowMat));

    const { rings, ringMats } = buildRings(group);
    const { nodes, nodePositions, connLines } = buildNodes(group, nodeMat);
    ...
    const updateColors = () => {
      ...
      nucleusMat.color.copy(accent); nucleusMat.emissive.copy(accent);
      glowMat.color.copy(accent); particleMat.color.copy(foreground); ptLight.color.copy(accent);
      nodes.forEach((n) => { (n.material as THREE.MeshBasicMaterial).color.copy(accent); (n.material as THREE.MeshBasicMaterial).needsUpdate = true; });
      connLines.forEach(({ line }) => { (line.material as THREE.LineBasicMaterial).color.copy(accent); (line.material as THREE.LineBasicMaterial).needsUpdate = true; });
      ringMats.forEach((m) => { m.color.copy(accent); m.needsUpdate = true; });
    ...
    const tick = () => {
      ...
      rings.forEach((ring) => {
        ...
        ring.scale.setScalar(0.5 + progress * 4.5);
