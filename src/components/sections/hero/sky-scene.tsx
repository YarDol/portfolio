...
import { STAR_COUNT, BRIGHT_STAR_COUNT, createStarGeometry } from "./lib/star-geometry";

function syncTheme(
  renderer: THREE.WebGLRenderer,
  starMat: THREE.PointsMaterial,
  brightMat: THREE.PointsMaterial,
) {
...
export function SkyScene() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (disposed) {
        colorTex.dispose(); normalTex.dispose(); renderer.dispose();
      ...
      const starMat = new THREE.PointsMaterial({ size: 0.13, transparent: true, opacity: 0.82, sizeAttenuation: true, color: 0xe8eeff });
      const stars = new THREE.Points(starGeo, starMat);
      sceneGroup.add(stars);

      const brightGeo = createStarGeometry(BRIGHT_STAR_COUNT, 77, 42, 80);
      const brightMat = new THREE.PointsMaterial({ size: 0.42, transparent: true, opacity: 0.92, sizeAttenuation: true, color: 0xfff4e0 });
      const brightStars = new THREE.Points(brightGeo, brightMat);
      sceneGroup.add(brightStars);

      syncTheme(renderer, starMat, brightMat);
      const themeObserver = new MutationObserver(() => syncTheme(renderer, starMat, brightMat));
      ...
      const tick = () => {
        ...
        sceneGroup.rotation.x += (-mouse.y * 0.048 - sceneGroup.rotation.x) * 0.028;
        sceneGroup.rotation.y += (mouse.x * 0.048 - sceneGroup.rotation.y) * 0.028;
      ...
      cleanupRef.current = () => {
        ...
        colorTex.dispose(); normalTex.dispose(); moonGeo.dispose(); moonMat.dispose();
        starGeo.dispose(); starMat.dispose(); brightGeo.dispose(); brightMat.dispose();
        renderer.dispose();
        if (renderer.domElement.parentNode === el) el.removeChild(renderer.domElement);
      };
    }
