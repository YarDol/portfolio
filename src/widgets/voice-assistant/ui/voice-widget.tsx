...
import { useVoiceChat, type TtsEngine } from "../model/use-voice-chat";
import { trackEvent } from "@/shared/lib/analytics";
import {
  VOICE_DISABLED,
  VOICE_LLM_DISABLED,
  VOICE_ELEVENLABS_DISABLED,
} from "@/shared/config";
...
export function VoiceWidget({ locale = "en" }: { locale?: string }) {
  ...
  const [ttsEngine, setTtsEngine] = useState<TtsEngine>(
    VOICE_ELEVENLABS_DISABLED ? "browser" : "browser",
  );
  ...
  const {
    ...
    stop,
    isSupported,
  } = useVoiceChat({ locale, ttsEngine, disabled: VOICE_LLM_DISABLED });
      ...
      <AnimatePresence>
        {isOpen && (
          <VoiceModal
            ...
            showTtsToggle={!VOICE_ELEVENLABS_DISABLED && !VOICE_LLM_DISABLED}
          />
        )}
      </AnimatePresence>
    </>
